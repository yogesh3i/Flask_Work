...
def passed(sname,marks):
    return f"<h1>Congrats!!,{sname} Your are Passed with {marks} marks</h1>"

# Endpoijnt for a failed 
@app.route("/failed/<sname>/<int:marks>")
def failed(sname,marks):
    return f"<h1>Sorry {sname},you are failed with {marks} marks</h1>"
...
def score(name,Marks):
    if Marks<30:
        time.sleep(1)
        return redirect(url_for("failed",sname=name,marks=Marks)) # once we have created this code should have this endpoint to redirect so write that endpoint 
    else:
        time.sleep(1)
        return redirect(url_for("passed",sname=name,marks=Marks)) # saname and marks are given so that we can mention the name into the redirected url
